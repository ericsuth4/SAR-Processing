...
def checkdir(directory, numfiles, key='2024*'):  
    ...
    datapaths = os.path.join(directory, key)
    datacollects = glob.glob(datapaths)

    goodlines = []
    badlines = []

    for run in datacollects:
        channel = os.path.join(run, 'chan0')

        if len(os.listdir(channel)) >= numfiles and 'chan0_Quick_look.png' not in os.listdir(run):
            goodlines.append(run)

        elif len(os.listdir(channel)) < numfiles and 'chan0_Quick_look.png' not in os.listdir(run):
            badlines.append(run)
...
def load_data_list(file_list, start_idx=0, stop_idx=-1, start_sample=0, stop_sample=0):
    ...
    for f in file_list[start_idx:stop_idx]:  ## iterate through files of given indeces
        temp_data = np.fromfile(f, dtype=np.int16)
        temp_data = temp_data.reshape(-1, n_samp) / (2**4)

        print(f'File Index: {idx} with shape {temp_data.shape}')
        file_data.append(temp_data)  ##  adding data to a list. file_data becomes a list of 2D matrices, each item corresponds to a single .dat file

    return file_data  ##  List of matrices, each matrix is half second of data

def join_data_list(file_data):

    total_pulses = 0
    for data in file_data:
        total_pulses += data.shape[0]

    n_samp = file_data[0].shape[1]
    idx = 0
    full_data = np.zeros((total_pulses, n_samp))  ## Allocating memory for the full data array
    for data in file_data:  ## iterate through items (2D matrices) adding them to specific indeces of full data matrix
        n_pulses = data.shape[0]
...
def write_annotation_file(directory, channel, timestamp, start_file, stop_file, vp, H, f0, fs, prf, az_BW, norm_factor, range_samples, az_samples, range_res, az_res):

    text = f'''
    
Processing Parameters for {channel} at {timestamp} using Range Doppler processor...
...
Normalization Factor... Data has been divided by this to fit into a float16
Norm Factor:        {norm_factor}

Some parameters useful for loading data later...
Samples:            {range_samples}           ; Range Samples
Lines:              {az_samples}              ; Azimuth Samples

Range Resolution:   {range_res} [m/pixel]
Azimuth Resolution: {az_res} [m/pixel]
...
def write_slc(focused, directory, channel, timestamp, start_file, stop_file, vp, H, f0, fs, prf, az_BW, range_res, az_res):
    ...
    norm_factor = (max(abs(focused.flatten())) / 2**15)
    focused = focused / norm_factor

    write_annotation_file(directory, channel, timestamp, start_file, stop_file, vp, H, f0, fs, prf, az_BW, norm_factor, range_samples, az_samples, range_res, az_res)
...
def fix_slc_resolution(focused, vp, prf, fs, L, fh, fl):
    c = 3e8
    
    daz = vp / prf   ## pixel spacing from digital data
    dR = c/(2*fs)   ## Range pixel spacing from digital data
    ...
    if targetAz >= daz:
        az_multilook = int(np.round(targetAz/daz))
        az_res = daz*az_multilook
    ...
    if targetRng >= dR:
        rng_multilook = int(np.round(targetRng/dR))
        range_res = dR*rng_multilook

    else: 
        rng_multilook = 1
        range_res = dR

    focused = multilook(focused, rng_multilook, az_multilook)

    return focused, range_res, az_res
...
def write_doppler_fit(doppler_fit, dopplerpath):
    ...
    doppler_fit = doppler_fit.astype(np.float32)
...
def load_doppler_fit(dopplerpath):
    ...
    with open(dopplerpath, 'rb') as f:
        doppler_fit = np.fromfile(f, dtype=np.float32, count=-1)
...
def compress(data, tp, fs, fl, fh, direction, window=True, plot=True):
    ...
    if direction == 'up':  ##  Generating sin and cosine up chirps
        cosChirp = chirp(t_ref, fl, t_ref[-1], fh, method='linear').astype(np.float32)
        sinChirp = chirp(t_ref, fl, t_ref[-1], fh, method='linear', phi=90).astype(np.float32)

    elif direction == 'down':  ##  Generating sin and cosine down chirps
        cosChirp = chirp(t_ref, fh, t_ref[-1], fl, method='linear').astype(np.float32)
        sinChirp = chirp(t_ref, fh, t_ref[-1], fl, method='linear', phi=90).astype(np.float32)
...
def rcmc(data, lambda_, fs_az, fs_rng, Rmin, Rmax, vp):
    ...
    print('Beginning range migration correction...')
    ...
    R = np.linspace(Rmin, Rmax, rng_samp, dtype=np.float32).reshape((-1, rng_samp))
    fn = (np.linspace(-fs_az/2, fs_az/2, az_samp, dtype=np.float32)**2).reshape((az_samp, -1))  ## squared because of ra>   
    Rfn = fn.dot(R).astype(np.float32)
    del(R)
    del(fn)
    Rfn = (lambda_**2 * Rfn) / (8*vp**2)

    Rfn = (2*Rfn*fs_rng) / 3e8
    Rfn =  Rfn.astype(np.complex64)
    print('Finished allocating some memory...')
    print(f"RAM Usage: {psutil.virtual_memory().percent} %")
    print('')
    # Rshift = x - np.array(shift_samples)[:, np.newaxis]
    ...
    # print(xin)
    print('Doing the range shift...')
    for i in range(az_samp):
        x = xin - Rfn[i]
        interp_func = interp1d(x, data[i], kind='linear', bounds_error=False, fill_value=1)
        Rfn[i] = interp_func(np.arange(rng_samp))
    
    return Rfn

################################  This function works but is EXTREMELY memory inefficient  ######################################################
...
#     return Haz
################################################ Function above here works well but is EXTREMELY memory inefficient  #########################################

def azimuth_compress(data, lambda_, fs, BW, doppler_centroids, Rmin, Rmax, vp):
    print("Azimuth compressing...")
    r_samps = data.shape[1]
    freq_samps = data.shape[0]
    print(data.dtype)
    ...
    window = np.hamming(window_samps)
    window = np.pad(window, (before, after), mode='minimum')
    window = np.tile(window, (r_samps, 1)).T.astype(np.float32)
    centroid_idx = (doppler_centroids*Hz_samp).astype(int)
    ...
    print('Created Magnitude window...')
    print(f"RAM Usage: {psutil.virtual_memory().percent} %")
    del(centroid_idx)
    ###  Creating matched filter phase matrix  ###
    # freq_vector = np.linspace(-fs/2, fs/2, freq_samps).reshape((freq_samps, -1))  # 1 dimensional vector
    ...
    freq_matrix = np.linspace(-fs/2, fs/2, freq_samps).reshape((freq_samps, -1))  # 1 dimensional vector
    doppler_centroids.reshape((-1, r_samps))

    print('Created Frequency Vector...')
    print(f"RAM Usage: {psutil.virtual_memory().percent} %")
 
    freq_matrix = freq_matrix - doppler_centroids ############# uncomment this line #################################
    np.power(freq_matrix, 2, out=freq_matrix).astype(np.float32)  ##  square the result from above

    print('Finished a few operations on frequency matrix...')
    print(f"RAM Usage: {psutil.virtual_memory().percent} %")

    R = np.linspace(Rmin, Rmax, r_samps)  # 1D vector
    # R = np.tile(R, (freq_samps, 1))  # 2D Matrix
    np.divide(2 * vp**2, lambda_ * R, out=R)

    print(f'RAM Usage: {psutil.virtual_memory().percent} %')

    freq_matrix = -1j*np.pi*freq_matrix.astype(np.complex64)
    gc.collect()

    print(f'RAM Usage: {psutil.virtual_memory().percent} %')

    print('trying to divide by 1D Range vector...')
    freq_matrix = freq_matrix / R ########## np.divide(freq_matrix, R, out=freq_matrix)
    del(R)
    ...
    np.exp(freq_matrix, out=freq_matrix)

    print(f"RAM Usage: {psutil.virtual_memory().percent} %")

    np.multiply(freq_matrix, window, out=freq_matrix)
    del(window)
